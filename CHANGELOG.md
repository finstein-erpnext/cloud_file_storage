...
### Changed
- The File controller is attached with `extend_doctype_class` on frappe v16, composing
  `ExtendedFile -> CloudFile -> File` instead of replacing the controller, so upstream File
  fixes still apply and another app can extend File alongside this one. v15 has no such hook
  and keeps `override_doctype_class`. Which is used is probed from the framework.

## [16.0.0] - 2026-09-07

First release of the **v16 line**. Same source tree as the 15.x line; the major tracks the
frappe major, as it does across this ecosystem (erpnext 15.x -> frappe v15).

### Added
- Support for frappe v16 on Python 3.14, verified against `v16.0.0-rc.1`: 1557 tests
  discovered, the same count as v15, with an identical residual failure set.
- Release automation the repo previously lacked -- semantic-release per version line,
  weekly hotfix->stable PRs, and label-driven backports.

### Fixed
- SQL functions passed as strings in `fields=`, which v16's rewritten SELECT parser rejects
  outright; replaced with the query builder.
- Migration job-id marker, now derived from `create_job_id` instead of hardcoding `::`
  (v16 namespaces with `||`, so the literal matched nothing and the workload-isolation gates
  passed on an empty set).
- A34 `fid` floor is read from the code object; v16's PEP 649 annotations make
  `inspect.signature` raise on 3.14.
- `install.py` tests for MariaDB explicitly rather than "not postgres", so v16's sqlite
  backend is not handed MariaDB-only DDL.

### Changed
- `frappe` window narrowed to `>=16.0.0,<17.0.0` for this line; `requires-python` is
  `>=3.14,<3.15`, which frappe v16 requires.
- `dispatch_tick` reports the scheduler tick and warns while a campaign runs: v16 raised the
  default from 60s to 240s, which quadruples stale-batch recovery latency silently.
